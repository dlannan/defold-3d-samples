...
function geom:GenerateCube(goname, sz, d )
	...
	for key, func in ipairs(targets) do
		...
		for a = -sz, sz-stepsize, stepsize do
			...
			for b = -sz, sz-stepsize, stepsize do
				...
				local switch = false
				if(key == 2 or key == 5 or key == 6) then switch = true end 			
	...
	for idx = 1, iverts do  
		...
		vertBuffer[vi* 3+1] = verts[vi*3+1]
		vertBuffer[vi* 3+2] = verts[vi*3+2]
		vertBuffer[vi* 3+3] = verts[vi*3+3]
		normBuffer[vi* 3+1] = normals[vi*3+1]
		normBuffer[vi* 3+2] = normals[vi*3+2]
		normBuffer[vi* 3+3] = normals[vi*3+3]
		uvBuffer[vi* 2+1] = uvs[vi*2+1]
		uvBuffer[vi* 2+2] = uvs[vi*2+2]
	end 
...
-- ------------------------------------------------------------------------------------------------------------
-- 
-- function geom:GeneratePlane( sx, sy, uvMult, offx, offy )
-- 
